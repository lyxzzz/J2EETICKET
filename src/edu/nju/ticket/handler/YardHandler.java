...
public class YardHandler implements HandlerInterceptor{
	@Override
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
		...
		else
		{
			...
			for(Entry<String,String> entry:cookies.entrySet())
			{
				...
				{
					if(!entry.getValue().equals(AccountType.yard.name()))
					{
						//initSession(session,request,response);
    ...
    public static void initSession(HttpSession session,HttpServletRequest request, HttpServletResponse response,Yard yard)
    {
		...
		Cookie typecookie = new Cookie("type",AccountType.yard.name());   // 新建Cookie
		typecookie.setMaxAge(Integer.MAX_VALUE);           // 设置生命周期为MAX_VALUE
		response.addCookie(typecookie);
		session.setAttribute("JSESSIONID", session.getId());
		session.setAttribute("key", key);
		session.setAttribute("type", AccountType.yard.name());
		session.setAttribute("object", yard);
    }
}
