...
public class ManagerHandler implements HandlerInterceptor{
	@Override
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
		...
		else
		{
			...
			for(Entry<String,String> entry:cookies.entrySet())
			{
				...
				{
					if(!entry.getValue().equals(AccountType.manager.name()))
					{
						//initSession(session,request,response);
    ...
    public static void initSession(HttpSession session,HttpServletRequest request, HttpServletResponse response,Manager manager)
    {
		...
		session.setAttribute("object", manager);
    }
}
